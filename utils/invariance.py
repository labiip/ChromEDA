...
class InvNet(nn.Module):
    def __init__(self, num_features, num_classes, beta=0.05, knn=6, alpha=0.01):
        ...
        self.num_classes = num_classes     #N张源域目标
    ...
    def forward(self, inputs, targets, epoch=None):
        ...
        inputs = ExemplarMemory(self.em, alpha=alpha)(inputs, targets)   #新的batch特征与memory里的特征的欧式向量
        ...
        if self.knn > 0 and epoch > 4:
            ...
            loss = F.cross_entropy(inputs, targets)  #与每个类别的距离，作交叉损失函数 与真标签越近，距离越大，损失越小
        return loss
    ...
    def smooth_hot(self, inputs, targets, k=6):
        ...
        targets_onehot.scatter_(1, index_sorted[:, 0:k], ones_mat * weights)#选取最近的k个作为最近邻
